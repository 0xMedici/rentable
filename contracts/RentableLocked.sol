...
contract RentableLocked is Initializable {
    ...
    function initialize(
        ...
        uint256 _price,
        uint256 _rentalLength
    ) external initializer {
        creator = _creator;
        currency = _paymentCurrency;
        cost = _price;
        startTime = block.timestamp;
        rentalLength = _rentalLength;
    }
    ...
    function updateCurrency(address _currency) external isOwner {
        ...
        require(!rentalPaid[(block.timestamp - startTime) / rentalLength]);
        currency = _currency;
    }
    ...
    function claimRentalPayment(uint256 epoch) external isOwner {
        require(epoch > ((block.timestamp - startTime) / rentalLength));
    ...
    function newRenter(address _new) external {
        ...
        rentalPaid[(block.timestamp - startTime) / rentalLength] = true;
        renter = _new;
    }
